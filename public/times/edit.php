<?php
// public/times/edit.php (bereinigt)
...
function is_safe_return($url) {
  if (!$url) return false;
  if (preg_match('~^(?:https?:)?//~i', $url)) return false; // keine externen/absoluten URLs
...
if (!$time) {
  flash('Zeit nicht gefunden.', 'danger');
  redirect(is_safe_return($return_to) ? $return_to : $fallback);
  exit;
}

if (strtolower((string)($time['status'] ?? '')) === 'abgerechnet') {
  flash('Dieser Zeiteintrag ist bereits <strong>abgerechnet</strong> und kann nicht mehr geändert werden.', 'danger');
  redirect(is_safe_return($return_to) ? $return_to : $fallback);
  exit;
}
...
// ---- Listen für abhängige Dropdowns ----
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($started_at === '') {
    flash('Startzeit ist erforderlich.', 'danger');
    redirect($_SERVER['REQUEST_URI']); // zurück zum Formular
    exit;
  }
  ...
  try {
    ...
    flash('Ungültiges Start-Datum/Zeitformat.', 'danger');
  ...
  if ($ended_at !== '') {
    try {
      $end = new DateTimeImmutable($ended_at);
    } catch (Throwable $e) {
      flash('Ungültiges Ende-Datum/Zeitformat.', 'danger');
      redirect($_SERVER['REQUEST_URI']);
      exit;
    }
    if ($end <= $start) {
      flash('Endzeit muss nach der Startzeit liegen.', 'danger');
      redirect($_SERVER['REQUEST_URI']);
      exit;
    }
  }
...
function fmt_dt_local($s) {
  ...
  // HTML datetime-local erwartet "YYYY-MM-DDTHH:MM"
  ...
  <div class="col-md-7">
    ...
    <form method="post">
      ...
      (function(){
        ...
        if (selCompany) elCompany.value = selCompany;
